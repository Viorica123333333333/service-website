...
// Every placeholder is clearly labeled "IMAGINE PLASATOARE" so it can
...
function buildSvg({ width, height, bg, accent, glyph, label }) {
  ...
  <text x="${width / 2}" y="${height - 28}" text-anchor="middle" font-family="Georgia, serif" font-size="${Math.max(14, width * 0.028)}" fill="${PALETTE.charcoal}" opacity="0.55">IMAGINE PLASATOARE — înlocuiți cu fotografie reală</text>
</svg>
`;
}
...
write(
  ...
  buildSvg({
    ...
    glyph: 'wardrobe',
    label: 'Dulap montat, imagine ilustrativă',
  }),
);

// Gallery
const galleryItems = [
  ['dulap-freestanding', 'wardrobe', 'Dulap freestanding, imagine ilustrativă'],
  ['dulap-glisant', 'wardrobe', 'Dulap cu uși glisante, imagine ilustrativă'],
  ['dulap-modular', 'wardrobe', 'Dulap modular, imagine ilustrativă'],
  ['pat-dublu', 'bed', 'Pat dublu, imagine ilustrativă'],
  ['noptiere', 'bed', 'Noptiere, imagine ilustrativă'],
  ['comoda', 'drawer', 'Comodă, imagine ilustrativă'],
  ['raft', 'shelf', 'Raft de bibliotecă, imagine ilustrativă'],
  ['birou', 'desk', 'Birou, imagine ilustrativă'],
];
...
// Wardrobe story
const storyItems = [
  ['nivelare', 'Nivelare, imagine ilustrativă'],
  ['aliniere', 'Aliniere uși, imagine ilustrativă'],
  ['interior', 'Interior organizat, imagine ilustrativă'],
  ['verificare', 'Verificare finală, imagine ilustrativă'],
];
for (const [name, label] of storyItems) {
  write(
    `${ROOT}story/${name}.svg`,
    buildSvg({ width: 800, height: 600, bg: PALETTE.sage, accent: PALETTE.forest, glyph: 'wardrobe', label }),
  );
}
...
write(
  ...
  buildSvg({
    ...
    glyph: 'portrait',
    label: 'Portret profesional, imagine ilustrativă',
  }),
);
