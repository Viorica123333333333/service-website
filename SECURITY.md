...
- The submit button is disabled and its label changes to "Sending…" while a request is in
...
### Prototype safeguard: the contact form is disabled in this deployment

- This repository is currently published as a **portfolio prototype**, not a live business.
  `src/components/Contact/Contact.tsx` sets a single `PROTOTYPE_FORM_DISABLED` flag to `true`,
  which renders every form control (including the submit button) with the native `disabled`
  attribute and adds a visible, accessible on-page notice explaining that the form does not
  accept input.
- A disabled submit button cannot be activated by a mouse click or the keyboard, so
  `handleSubmit` can never fire through ordinary use. The handler also carries its own
  early-return guard on the same flag, as defence-in-depth against any non-standard way it
  could otherwise be invoked. No visitor can cause a real network request or submit real
  personal data through this deployment.
- The full validation and Netlify Forms submission logic is left intact and readable in the
  same file, specifically so it demonstrates a genuine, working implementation to anyone
  reviewing the code. Flip the flag to `false` (and complete the rest of
  `PRE_LAUNCH_CHECKLIST.md`) before using this project as a real business's website.
...
- `Content-Security-Policy` — frame protection is handled via `frame-ancestors 'none'`
  ...
  - `script-src 'self'` — only same-origin scripts execute. The one tiny inline-looking
    ...
    needed for scripts.
  - `style-src 'self' https://fonts.googleapis.com` — no `'unsafe-inline'`. All component
    styling lives in CSS Modules; the few spots that used to set an inline `style` attribute
    (a heading's spacing in `Contact.tsx`, an icon rotation in `Lightbox.tsx`) were moved into
    CSS Module classes specifically so this directive could stay tight. Keep new inline
    `style` props out of the codebase, or this directive will need loosening again.
...
- `package-lock.json` is committed to the repository, so every install (local, CI, or
  Netlify's build) resolves the exact same dependency tree rather than whatever the loosest
  version range would currently allow.
...
- The frontend never displays raw error objects, stack traces, or internal state to users —
  form failures show one generic, friendly message ("Something went wrong while submitting
  the form…") alongside a phone/email fallback.
